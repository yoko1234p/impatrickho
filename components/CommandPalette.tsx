...
export const CommandPalette = () => {
  ...
  const actions = [
    ...
    },
    {
      ...
      action: () => {
        const wasRecruiterMode = isRecruiterMode;
        if (!wasRecruiterMode) {
          useStore.getState().setRecruiterMode(true);
        }
        setIsOpen(false);

        // Add print class to force all animations to complete state
        document.body.classList.add('print-mode');

        // Wait for DOM update then print
        setTimeout(() => {
          window.print();
          // Restore original mode and remove print class after print
          document.body.classList.remove('print-mode');
          if (!wasRecruiterMode) {
            useStore.getState().setRecruiterMode(false);
          }
        }, 300);
      }
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-[#111] border border-[#333] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center p-4 border-b border-[#333]">
          ...
          />
          <button onClick={() => { setIsOpen(false); setSearch(''); }} className="text-gray-500 hover:text-white">
            <X size={20} />
          </button>
        </div>
        <div className="p-2">
